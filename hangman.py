...
def get_valid_word(words):
    ...
    while '-' in word or ' ' in word:  # Avoid words with dashes or spaces
...
def display_hangman(lives):
    ...
    return hangman_stages[7 - lives]  # Index to match the number of lives remaining

def hangman():
    word = get_valid_word(words).upper()  # Convert the word to uppercase
    ...
    lives = 7  # Number of allowed incorrect guesses
    ...
    while len(word_letters) > 0 and lives > 0:
        ...
        user_letter = input("Guess a letter: ").upper()  # Take user input and convert to uppercase
        
        # Check if the guessed letter is in the alphabet and not yet guessed
        if user_letter in alphabet - used_letters:
            used_letters.add(user_letter)  # Add the guessed letter to the used letters set
            
            if user_letter in word_letters:  # If the guessed letter is in the word
                word_letters.remove(user_letter)  # Remove it from the letters to guess
            else:
                lives -= 1  # If the guessed letter is not in the word, reduce the lives count
        ...
        # If the input is not a valid alphabet letter
        else:
            print("You have typed an invalid character. Please try again.")

    # End game messages
    if lives == 0:
        print(display_hangman(lives))  # Display the final hangman stage
